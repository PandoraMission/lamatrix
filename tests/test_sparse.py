...
def test_sparse_fits():
    ...
    assert np.allclose(
        model.to_gradient().design_matrix(x=x.toarray()[:, 0]),
        model.to_gradient().design_matrix(x=x).toarray(),
    )
    ...
    model.fit(
        ...
        errors=sparse.csr_matrix(errors).T,
        mask=mask,
    )
    assert np.allclose(model.posteriors.mean, w)
...
def test_sparse_combine():
    ...
    dm_sparse = model.design_matrix(
        Z=sparse.csr_matrix(Z.ravel()).T, C=sparse.csr_matrix(C.ravel()).T
    )
