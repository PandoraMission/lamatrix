...
class DistributionsContainer:
    ...
    def __init__(self, distributions: List[Tuple]):
        if not isinstance(distributions, list) or not all(isinstance(d, tuple) and len(d) == 2 for d in distributions):
            raise ValueError("Distributions should be a list of tuples, each containing (mean, std)")
        
        # Validate each distribution
        for mean, std in distributions:
            if not (isinstance(mean, (int, float, np.integer, np.number)) and isinstance(std, (int, float, np.integer, np.number))):
        ...
        if isinstance(index, slice):
            ...
            if (index.start is None) & (index.stop is None) & (index.step is None):
                if not len(value) == self._length:
                    raise ValueError("Replacement value must be a list with the same length as the slice")
                for idx in range(self._length):
                    self._distributions[idx] = Distribution(value[idx])
                return

            # Check there are enough values
            if not isinstance(value, list) or len(value) != len(range(*index.indices(self._length))):
                raise ValueError("Replacement value must be a list with the same length as the slice")

            idxs = np.arange(index.start if index.start is not None else 0, index.stop if index.stop is not None else self._length, index.stp if index.step is not None else 1)
